...
fn bench(c: &mut Criterion) {
    const INPUT: &'static str = include_str!("../src/input.txt");

    let mut group = c.benchmark_group("Part 1");
    group.bench_function("Assembly", |b| b.iter(|| assembly1(black_box(INPUT))));
    group.bench_function("Functional", |b| b.iter(|| part1(black_box(INPUT))));
    group.finish();

    let mut group = c.benchmark_group("Part 2");
    group.bench_function("Assembly", |b| b.iter(|| assembly2(black_box(INPUT))));
    group.bench_function("Functional", |b| b.iter(|| part2(black_box(INPUT))));
    group.finish();
}
